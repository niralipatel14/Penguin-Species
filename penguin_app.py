...
flipper_len = st.sidebar.slider("flipper_length_mm", float(df["flipper_length_mm"].min()), float(df["flipper_length_mm"].max()))
...
classifier_type = st.sidebar.selectbox("Classifier", ("Support Vector Machine", "Logistic Regression", "Random Forest Classifier"))

if st.sidebar.button("Predict"):
  if classifier_type == "Support Vector Machine":
    species_type = prediction(svc_model, select_island, bill_len, bill_dep, flipper_len, body_mass, select_sex)
    score = svc_model.score(X_train, y_train)
  elif classifier_type == "Logistic Regression":
    species_type = prediction(log_reg, select_island, bill_len, bill_dep, flipper_len, body_mass, select_sex)
    score = log_reg.score(X_train, y_train)
  else:
    species_type = prediction(rf_clf, select_island, bill_len, bill_dep, flipper_len, body_mass, select_sex)
